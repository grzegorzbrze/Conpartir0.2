    ...
    modService.factory('shared', ['$http', '$location', 'auth',
        function ($http, $location, auth) {
            ...
            return {
                ...
                deleteEmail: function (input) {
                    var res;
                    var sr;
                    var action;
                    var opName;
                    var promise;
                    var opName = "deleteSecondaryEmail";
                    sr = SOAPhead +
                            '<ns0:' + opName + ' xmlns:ns0="http://SOAPServer/">' +
                            '<email>' + input.email + '</email>' +
                            '<secondaryEmail>' + input.secondaryEmail + '</secondaryEmail>' +
                            '</ns0:' + opName + '>' +
                            SOAPtail;
                    action = '"' + "http://SOAPServer" + "/" + opName + '"';
                    ...
                    promise = $http.post(SOAPbase, sr, {"headers": {
                            ...
                            .success(function (data, status, headers, config) {
                                var jsonObj = x2js.xml_str2json(data);
                                res = jsonObj.Envelope.Body.deleteSecondaryEmailResponse;
                                delete res["_xmlns:ns2"];
                                delete res["__prefix"];
                                obj = res;
                            })
